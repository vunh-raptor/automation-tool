...
def general_request_reqdata_SINGLE(jira_session: JiraSession, origin_ticket_key: str) -> list:
    ...
    for id in approval_id_list:
        ticket_data = jira_session.browse_ticket(
            ticket_key=id, param1="resolution", param2="summary")
        # This IF will filter out the ticket that been declined
        if ticket_data.get_resolution() == "Resolved":
            return_list.append(ticket_data.get_summary().replace(
                'Service Request - L2 - "Role Approver - Role" - "', "").replace('"', "") + " - " + "Approved" + " - " + affected_account)
        else:
            return_list.append(ticket_data.get_summary().replace(
                'Service Request - L2 - "Role Approver - Role" - "', "").replace('"', "") + " - " + "Declined" + " - " + affected_account)
    return return_list
